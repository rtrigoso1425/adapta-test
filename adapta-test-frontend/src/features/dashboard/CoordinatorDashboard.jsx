  ...
  return (
    ...
    >
      ...
      <form onSubmit={onSubmit}>
        ...
        <div style={{ marginBottom: "15px" }}>
          <label htmlFor="cycle-number">Número de Ciclo:</label>
          <br />
          <input
            id="cycle-number"
            type="number"
...
const SyllabusUploader = ({ coursesInCurriculum }) => {
  ...
  return (
    <div
      style={{ border: "2px dashed #ccc", padding: "20px", marginTop: "30px" }}
    >
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ marginBottom: "15px" }}>
          <label>Archivo PDF del Sílabus:</label>
          <br />
          <input
            type="file"
            accept=".pdf"
...
const CoordinatorDashboard = () => {
  ...
  // Obtenemos la carrera asignada desde el 'careerSlice'
  const { myCareer, isLoading } = useSelector((state) => state.careers);

  useEffect(() => {
    // Pedimos la carrera del coordinador. El backend la filtrará por institución.
    dispatch(getMyCareer());
    // También necesitamos la lista de todos los cursos de la institución para poder añadirlos a la malla.
    dispatch(getCourses());
  ...
  // La lógica para extraer los cursos de la malla se mantiene
  const allCoursesInCurriculum = myCareer.curriculum
    ? myCareer.curriculum.flatMap((cycle) => cycle.courses)
    : [];
