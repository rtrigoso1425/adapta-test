  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(startEvaluation.fulfilled, (state, action) => {
        ...
        // CORRECCIÓN 1: Inicializamos el score en 0 explícitamente al empezar
        state.session = { 
      ...
      .addCase(submitAnswer.fulfilled, (state, action) => {
        ...
        if (action.payload.status === 'completed') {
          // Si terminó, usamos el objeto session completo que devuelve el backend
          state.isCompleted = true;
          state.currentQuestion = null;
          state.session = action.payload; 
        } else {
          // Si NO terminó, avanzamos la pregunta...
          state.currentQuestion = action.payload.nextQuestion;

          // CORRECCIÓN 2: ¡Actualizamos el score manualmente!
          // Como sabemos si fue correcta o no gracias a 'lastResult', sumamos aquí mismo.
          
          // Aseguramos que existe la estructura por si acaso
          if (!state.session.score) {
             state.session.score = { correct: 0, incorrect: 0 };
          }

          if (action.payload.result.isCorrect) {
             state.session.score.correct += 1;
          } else {
             state.session.score.incorrect += 1;
          }
        }
      })
