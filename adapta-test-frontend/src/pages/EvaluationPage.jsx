...
import { startEvaluation, submitAnswer, reset } from '../features/evaluation/evaluationSlice';

const EvaluationPage = () => {
    const { moduleId } = useParams();
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const { session, currentQuestion, lastResult, isCompleted, isLoading, isError, message } = useSelector((state) => state.evaluation);
                ...
                <div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        {currentQuestion.options.map((option) => (
                            <button key={option._id} onClick={() => handleAnswerSubmit(option._id)} disabled={isLoading}>
                                {option.text}
                            </button>
                        ))}
                    </div>
                    ...
                    {lastResult && (
                        <p style={{ fontWeight: 'bold', color: lastResult.isCorrect ? 'green' : 'red' }}>
                            Tu última respuesta fue: {lastResult.isCorrect ? '¡Correcta!' : 'Incorrecta'}
                        </p>
                    )}
                </div>
            ) : (
